...
var CATCH_SOUND = loadSound("bomb_catch_shorter.wav");
var EXPLODE_SOUND = loadSound("bomb_explode.wav");
var BOMB_SPRITE = loadImage("bomb.png");
var EXPLODE_SPRITE = loadImage("bomb_explode.png");
var TITLE = loadImage("quick_title.png");
if (Math.random() > .8)
...
function onSetup() {
    ...
    bucket.sprite = loadImage("bucket.png");
    ...
    bomber.sprite = loadImage("bomber.png");
...
function animate() {
  ...
  if (game.state < 0) {
    if ((bucket.key.left.pressed)||(bucket.key.right.pressed)) game.state = 0;
  }
  else if (!game.state) {
    // user controls
    ...
    //if (bucket.pos.x < bucket.width) { bucket.pos.x = bucket.width/2;}
    //if (bucket.pos.x > screenWidth) { bucket.pos.x = screenWidth;}
    //if (bucket.pos.x < 0) { bucket.pos.x = 0;}
    //if (bucket.pos.x > screenWidth) { bucket.pos.x = screenWidth;}
    ...
    for (i=0;i<BOMBS_MAX;i++) {
        ...
        if (caught) {
            ...
            score += game.stage + 1;
            bombs[i].active = false;
            // issue: two sounds cancel when they play close together in time (not sure how to multithread in js
  ...
  else if (game.state == 1)
  {
      ...
      if (max_i < 0) game.state = 2;
          ...
          //playSound(EXPLODE_SOUND);
          //bombs[max_i].color = makeColor(1,1,1);
...
function display() {
    ...
    drawImage(bomber.sprite,bomber.pos.x-bomber.width/2,bomber.pos.y-bomber.height/2);
